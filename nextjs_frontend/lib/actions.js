...
// Make sure this is in lib/actions.js
export async function deleteBookAction(bookId) {
  ...
  console.log("DEBUG: Attempting to delete book ID:", bookId);

  if (!token) {
    throw new Error("No authentication token found.");
  }

  // Ensure WP_URL is set correctly in your .env.local
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_WP_URL}/wp/v2/book/${bookId}?force=true`,
    {
      method: "DELETE",
      headers: {
